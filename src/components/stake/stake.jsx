...
class Stake extends Component {
  ...
  getWalletRewardBalance = (pool, account) => {
    ...
    return (
      <div className={classes.root}>
        ...
        <div className={classes.overview}>
          ...
          <div className={classes.overviewField}>
        <Typography variant={'h3'} className={classes.overviewTitle}>{pool.tokens[0].symbol != '$' ? pool.tokens[0].symbol : ''} Balance</Typography>
            <Typography variant={'h2'} className={classes.overviewValue}>{pool.tokens[0].balance ? pool.tokens[0].balance.toFixed(2) : "0"} </Typography>
          </div>
          <div className={classes.overviewField}>
            <Typography variant={'h3'} className={classes.overviewTitle}>{t('Stake.CurrentlyStaked')}</Typography>
            <Typography variant={'h2'} className={classes.overviewValue}>{this.getCurrentlyStaked() ? this.getCurrentlyStaked().toFixed(2) : "0"}</Typography>
          </div>
          <div className={classes.overviewField}>
            <Typography variant={'h3'} className={classes.overviewTitle}>{t('Stake.RewardsAvailable')}</Typography>
            <Typography variant={'h2'} className={classes.overviewValue}>{pool.tokens[0].rewardsSymbol == '$' ? pool.tokens[0].rewardsSymbol : ''} {this.getCurrentRewardsAvailable() ? this.getCurrentRewardsAvailable().toFixed(2) : "0"} {pool.tokens[0].rewardsSymbol != '$' ? pool.tokens[0].rewardsSymbol : ''}</Typography>
          </div>
        </div>
        {pool.id === 'Fee Rewards' &&
  ...
  getCurrentlyStaked = () => {
    ...
    const asset = pool.tokens;
    let balance = 0;
    console.log("GETTING STAKED BALANCE");
    asset.map((assets) => {
      console.log(assets);
      balance = balance + assets.stakedBalance;
    })
    return balance;
  }

  getCurrentRewardsAvailable = () => {
    const {  pool } = this.state
    const asset = pool.tokens;
    let balance = 0;
    console.log("GETTING current rewards BALANCE");
    asset.map((assets) => {
      console.log(assets);
      balance = balance + assets.rewardsAvailable;
    })
    console.log(balance);
    return balance;
  }

  renderStakePools = (assets) => {
    const { classes, t } = this.props;
  ...
  renderStake = () => {
    ...
    const asset = pool.tokens;
    console.log(asset);
    return asset.map((assets) => {
      return this.renderStakePools(assets)
    })
  ...
  renderUnstake = () => {
    ...
    console.log(asset);
    return asset.map((assets) => {
      return this.renderUnstakepools(assets)
    })
  }

  renderUnstakepools = (assets) => {
    const { classes, t } = this.props;
  ...
  _onClaim = (asset) => {
    ...
    this.setState({ loading: true })
    dispatcher.dispatch({ type: GET_REWARDS, content: { asset: asset } })
  }
    ...
    return (
      <div className={classes.valContainer} key={asset.id + '_' + type}>
        <div className={classes.balances}>
          {type === 'stake' && <Typography variant='h4' onClick={() => { this.setAmount(asset.id, type, (asset ? asset.balance : 0)) }} className={classes.value} noWrap>{'Balance: ' + (asset && asset.balance ? (Math.floor(asset.balance * 10000000000000000) / 10000000000000000).toFixed(16) : '0.0000')} {asset ? asset.symbol : ''}</Typography>}
          {type === 'unstake' && <Typography variant='h4' onClick={() => { this.setAmount(asset.id, type, (asset ? asset.stakedBalance : 0)) }} className={classes.value} noWrap>{'Balance: ' + (asset && asset.stakedBalance ? (Math.floor(asset.stakedBalance * 10000000000000000) / 10000000000000000).toFixed(16) : '0.0000')} {asset ? asset.symbol : ''}</Typography>}
        </div>
        <div>
          <TextField
            fullWidth
